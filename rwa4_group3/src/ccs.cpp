...
CCS::CCS(std::string node_name) : Node(node_name) {
  ...
  robot_task_publisher_ = this->create_publisher<rwa4_group3::msg::RobotTask>(
      "robot_task", 10);

  robot_status_publisher_ = this->create_publisher<rwa4_group3::msg::RobotStatus>(
      "robot_status", 10);

  robot_status_subscriber_ = this->create_subscription<rwa4_group3::msg::RobotStatus>(
      "robot_status", 10,
...
//=================================================================================================
void CCS::UpdateRobotStatus(const rwa4_group3::msg::RobotStatus::SharedPtr msg) {
  // RCLCPP_INFO_STREAM(this->get_logger(), "Robot Status Updated");
  floor_robot_ = msg->floor_robot;
  ceiling_robot_ = msg->ceiling_robot;
  if(floor_robot_ == rwa4_group3::msg::RobotStatus::FREE){
...
//=================================================================================================
std::vector<rwa4_group3::msg::RobotTask> CCS::CreateRobotTask(Order o){
  // Create a vector to store the tasks
  std::vector<rwa4_group3::msg::RobotTask> tasks;

  if(o.type == ariac_msgs::msg::Order::KITTING){

      // Create a task object
      rwa4_group3::msg::RobotTask tray_task;
      // Get AGV Number
      tray_task.agv_number = o.agv_number;
      // Get Tray ID
      tray_task.tray_id = o.kitting_task.tray_id;
      // Set the task type
      tray_task.task_type = rwa4_group3::msg::RobotTask::TRAY;
    ...
    for (auto part : o.kitting_task.parts) {
        ...
        // Create a task object
        rwa4_group3::msg::RobotTask part_task;

        // Set the task type
        part_task.task_type = rwa4_group3::msg::RobotTask::PART;
...
void CCS::ExecuteTasks(std::vector<rwa4_group3::msg::RobotTask> tasks, Order o){
    RCLCPP_INFO_STREAM(this->get_logger(),CYAN<<"Order Execution "<< o.id <<RESET);
    for (auto task : tasks) {
          while(task_in_waiting_ > 0){
              // RCLCPP_INFO_STREAM(this->get_logger(),RED<<"Waiting for the robot to be free"<<RESET);
          }
         if(o.priority==0){
          RCLCPP_INFO_STREAM(this->get_logger(),RED<<"There are "<<priority_order_count_<<" priority orders"<<RESET);
          while(priority_task_count_ > 0){
              
          }
         }
         RCLCPP_INFO_STREAM(this->get_logger(),ORANGE<<"Robot Status Before Task Execution "<<floor_robot_<<RESET);
         task_in_waiting_ ++;
         while (floor_robot_ == rwa4_group3::msg::RobotStatus::OCCUPIED){

         }
         rwa4_group3::msg::RobotStatus robot_status;
        robot_status.floor_robot = rwa4_group3::msg::RobotStatus::OCCUPIED;
        robot_status_publisher_->publish(robot_status);
        floor_robot_ = rwa4_group3::msg::RobotStatus::OCCUPIED;
        RCLCPP_INFO_STREAM(this->get_logger(),RED<<"Robot Status: OCCUPIED"<<RESET);
         
         RCLCPP_INFO_STREAM(this->get_logger(), CYAN<<"======================== " << o.id << " ========================"<<RESET);
         
         if(task.task_type == rwa4_group3::msg::RobotTask::PART){
            // continue;
         ...
         else if(task.task_type == rwa4_group3::msg::RobotTask::TRAY){
